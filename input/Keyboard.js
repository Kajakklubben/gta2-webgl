(function(){
	...
	GTA.Input.Keyboard.prototype = {
		...
		constructInputBitmask: function()
		{
			...
			// Check each key
			if(this.keys['up']) input |= GTA.Constants.INPUT_BITMASK.UP;
			if(this.keys['down']) input |= GTA.Constants.INPUT_BITMASK.DOWN;
			if(this.keys['left']) input |= GTA.Constants.INPUT_BITMASK.LEFT;
			if(this.keys['right']) input |= GTA.Constants.INPUT_BITMASK.RIGHT;
			if(this.keys['space']) input |= GTA.Constants.INPUT_BITMASK.SPACE;
			if(this.keys['shift']) input |= GTA.Constants.INPUT_BITMASK.SHIFT;
			if(this.keys['tab']) input |= GTA.Constants.INPUT_BITMASK.TAB;
		...
		{
			this.keys['up'] = (inputBitmask & GTA.Constants.INPUT_BITMASK.UP);
			this.keys['down'] = (inputBitmask & GTA.Constants.INPUT_BITMASK.DOWN);
			this.keys['left'] = (inputBitmask & GTA.Constants.INPUT_BITMASK.LEFT);
			this.keys['right'] = (inputBitmask & GTA.Constants.INPUT_BITMASK.RIGHT);
			this.keys['space'] = (inputBitmask & GTA.Constants.INPUT_BITMASK.SPACE);
			this.keys['shift'] = (inputBitmask & GTA.Constants.INPUT_BITMASK.SHIFT);
		},
